  ...
  > extends BaseTypesVisitor<TRawConfig, TParsedConfig> {
	...
	EnumTypeDefinition(node: EnumTypeDefinitionNode): string {
	  ...
	  if (this.config.enumsAsConst) {
		const typeName = `export type T${enumTypeName} = typeof ${enumTypeName}[keyof typeof ${enumTypeName}];`;
				...
				node.values
					.map((enumOption) => {
						...
						const comment = transformComment(
							enumOption.description as any as string,
							1
						);
						const name = enumOption.name as unknown as string;
						const enumValue: string | number =
							getValueFromConfig(name) ?? name;

						return (
							comment +
							indent(
								`${optionName}: { key: ${wrapWithSingleQuotes(
									name
								)}, ${wrapWithSingleQuotes(
									enumValue
								)}, display: t\`${wrapWithSingleQuotes(
									enumValue
								)}\` }`
							)
						);
